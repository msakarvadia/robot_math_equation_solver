...
```
-   Once the program has ran the inference on an image taken from the camera, it will display how it has interpreted the math problem through a series of popups (these can be used to inspect if the handwriting is being read correctly and if the camera is properly positioned)
